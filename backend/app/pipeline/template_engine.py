...
class DynamicSQLGenerator:
    ...
    def generate(self, nlp_data: Dict[str, Any], schema_map: Dict[str, List[Dict[str, str]]]) -> Optional[str]:
        """
        ...
        if intent == "list":
            ...
            return self._build_select(table_name, cols, where_clauses, sort_order, effective_limit, numeric_columns)
            ...
            metric_col = self._resolve_metric_column(nouns, all_columns, numeric_columns)
            if not metric_col:
                return None
            group_col = self._resolve_column(grouping_keys, all_columns, quote=True)
            if group_col:
            ...
            return self._build_trend(table_name, date_col, metric_col, granularity, where_clauses, effective_limit)
    ...
    def _build_select(self, table: str, cols: str, where: List[str],
                      sort_order: str | None, limit: int, numeric_cols: List[str]) -> str:
        where_str = f" WHERE {' AND '.join(where)}" if where else ""
        order_str = ""
        if sort_order and numeric_cols and cols == "*":
            # Only sort by numeric column when selecting all columns
            order_str = f' ORDER BY "{numeric_cols[0]}" {sort_order}'
    ...
    def _build_trend(self, table: str, date_col: str, metric_col: str | None,
                     granularity: str, where: List[str], limit: int) -> str:
        where_str = f" WHERE {' AND '.join(where)}" if where else ""
        trunc = f"DATE_TRUNC('{granularity}', \"{date_col}\")"
        if metric_col:
            return (
                f'SELECT {trunc} as period, COUNT(*) as count, SUM({metric_col}) as total '
                f'FROM "{table}"{where_str} '
                f'GROUP BY period ORDER BY period LIMIT {limit}'
            )
        return (
    ...
    def _resolve_metric_column(self, nouns: List[str], all_columns: List[str],
                                numeric_columns: List[str]) -> Optional[str]:
        """Resolve a numeric metric column — prefer explicit match, fall back to first numeric."""
        col = self._resolve_column(nouns, numeric_columns, quote=True)
        if col:
            return col
        # Fall back to first numeric column
        if numeric_columns:
            return f'"{numeric_columns[0]}"'
        return None
    ...
    def _resolve_filters(self, entities: Dict[str, Any], columns: List[str],
                         nlp_filters: List[Dict]) -> List[str]:
        filters = []

        # 1. Date filters from entities
        ...
        if dates:
            ...
            if date_col:
                ...
                if len(target_date) == 4 and target_date.isdigit():
                    ...
                    filters.append(f'"{date_col}"::text LIKE \'%{target_date}%\'')
